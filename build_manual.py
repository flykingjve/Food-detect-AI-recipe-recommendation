...
h1("三、環境需求與安裝")
h2("環境需求")
bullet("作業系統：Windows（macOS / Linux 亦可，啟動指令略有不同）")
bullet("Python：3.11 以上")
bullet("GPU（建議）：NVIDIA GPU + CUDA（可選，CPU 亦可執行）")
bullet("攝影機功能：需本機有可用 webcam")
note("本庫已附訓練好的 weights/best.pt，clone 後安裝套件即可執行，不需重新訓練或下載資料集。")
h2("安裝步驟（從 GitHub clone）")
p("1. 取得專案：")
code("git clone https://github.com/flykingjve/Food-detect-AI-recipe-recommendation.git\ncd Food-detect-AI-recipe-recommendation")
p("2. 建立並啟動虛擬環境（首次）：")
code("python -m venv venv\nvenv\\Scripts\\activate")
p("3. 安裝套件（預設 CPU 版 torch；GPU 另裝對應 CUDA 版）：")
code("pip install -r requirements_backend.txt\n:: GPU 例（CUDA 12.4）：\n:: pip install torch torchvision --index-url https://download.pytorch.org/whl/cu124")
p("4. 設定 API 金鑰（從範本複製後填入自己的金鑰）：")
